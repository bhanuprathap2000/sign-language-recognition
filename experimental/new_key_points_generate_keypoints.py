...
def process_landmarks(landmarks):
    '''
    landmarks are the results of the mediapipe model for a single frame
    and landmarks.landmark is a list of landmarks for a single hand.
    we are creating a list of x and y coordinates for each landmark
    and appending them to the list of x and y coordinates for the hand and returning them
    '''
    x_list, y_list = [], []
...
def process_hand_keypoints(results):
    '''
    results is the results of the mediapipe model for a single frame
    and results.multi_hand_landmarks is a list of hand_landmarks for a single hand.
    we are creating a list of hand1 and hand2 x and y coordinates for each landmark
    and appending them to the list of x and y coordinates for the hand and returning them
    '''
...
def process_pose_keypoints(results,path):
...
def process_video(path, save_dir):
    ...
    while cap.isOpened():
        ...
        pose_x, pose_y = process_pose_keypoints(pose_results,path)
